package com.supe.supertest.wxapi;
...
import com.supe.supertest.http.WXAccessTokenEntity;
import com.supe.supertest.http.WXBaseRespEntity;
...
public class WXEntryActivity extends AppCompatActivity implements IWXAPIEventHandler {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        try {
            ...
            if(!result){
                L.d("WXEntry_LOG","参数不合法，未被SDK处理，退出");
                finish();
            }
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public void onReq(BaseReq baseReq) {
        L.d("WXEntry_LOG","baseReq:"+ JSON.toJSONString(baseReq));
    }

    @Override
    public void onResp(BaseResp baseResp) {
        L.d("WXEntry_LOG:--A",JSON.toJSONString(baseResp));
        L.d("WXEntry_LOG--B:",baseResp.errStr+","+baseResp.openId+","+baseResp.transaction+","+baseResp.errCode);
...
//                showMsg(1,result);
                ...
                Log.i("WXEntry_LOG",entity.getCode()+"_______________");
                OkHttpUtils.get().url("https://api.weixin.qq.com/sns/oauth2/access_token")
                        .addParams("appid",WX_ID)
                        .addParams("secret",WX_SECRET)
                        ...
                        .execute(new StringCallback() {
                            ...
                            public void onError(okhttp3.Call call, Exception e, int id) {
                                L.d("WXEntry_LOG","请求错误..");
                            }

                            @Override
                            public void onResponse(String response, int id) {
                                L.d("WXEntry_LOG:",response);
...
//                                    getUserInfo(accessTokenEntity);
            ...
            case BaseResp.ErrCode.ERR_USER_CANCEL:
                result = "发送取消";
                L.d("WXEntry_LOG","发送取消");
                finish();
                break;
            case BaseResp.ErrCode.ERR_AUTH_DENIED:
                result = "发送被拒绝";
                L.d("WXEntry_LOG","发送被拒绝");
                finish();
                break;
            case BaseResp.ErrCode.ERR_BAN:
                result = "签名错误";
                L.d("WXEntry_LOG","签名错误");
                break;
            default:
                result = "发送返回";
//                showMsg(0,result);
    ...
    private void getUserInfo(WXAccessTokenEntity accessTokenEntity) {
        ...
        OkHttpUtils.get()
                ...
                .execute(new StringCallback() {
                    @Override
                    public void onError(okhttp3.Call call, Exception e, int id) {
                        L.d("WXEntry_LOG","获取错误..");
                    }

                    @Override
                    public void onResponse(String response, int id) {
                        L.d("WXEntry_LOG","userInfo:"+response);
